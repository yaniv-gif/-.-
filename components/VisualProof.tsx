  ...
  return (
    <section className="py-24 bg-white border-b border-stone-200">
      <div className="container mx-auto px-6">
        ...
        <div className="flex flex-col lg:flex-row gap-12 lg:gap-24 min-h-[600px] items-center">
          
          {/* Left Side: The Menu (Sticky Interaction) */}
            ...
            <div className="space-y-8">
              {projects.map((project) => (
                <div 
                  ...
                  className={`group cursor-pointer transition-all duration-500 border-l-2 pl-6 ${activeId === project.id ? 'border-bronze-500 translate-x-[-10px]' : 'border-stone-100 hover:border-stone-300'}`}
                >
                  <div className={`text-xs font-bold tracking-widest uppercase mb-2 transition-colors duration-300 ${activeId === project.id ? 'text-bronze-600' : 'text-stone-400'}`}>
                    {project.category}
                  </div>
                  <h3 className={`text-2xl md:text-3xl font-serif transition-colors duration-300 flex items-center gap-4 ${activeId === project.id ? 'text-stone-900' : 'text-stone-300 group-hover:text-stone-500'}`}>
                    {project.title}
                  </h3>
                </div>
              ))}
          ...
          {/* Right Side: The Visual (Interactive Canvas) */}
          <div className="w-full lg:w-2/3 h-[400px] lg:h-[600px] relative rounded-sm overflow-hidden shadow-2xl shadow-stone-200 order-1 lg:order-2 reveal delay-100">
            {projects.map((project) => (
              <div 
                key={project.id}
              ...
              >
                ...
                <div className="absolute bottom-8 right-8 text-white translate-y-4 opacity-0 transition-all duration-700 delay-300" style={{ opacity: activeId === project.id ? 1 : 0, transform: activeId === project.id ? 'translateY(0)' : 'translateY(20px)' }}>
